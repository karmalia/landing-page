...
const BackEndTechStack = [
  {
    label: "NodeJS",
    src: "/tech/icons8-nodejs-480.png",
  },
  {
    label: "ExpressJS",
...
function Projects({}: Props) {
  ...
  return (
    <SteamWindow variant="inner" className="px-2 py-1 space-y-2">
      <SteamText variant="goldLight">
        <span className="text-lg">My Tech Stack</span>
      </SteamText>
      <SteamWindow className="flex flex-col justify-between p-2 ">
        ...
        <div className=" flex flex-wrap gap-4 p-2 h-auto max-h-[400px] duration-300 ease-linear">
          {currentTechStack.stack.map((tech, index) => (
            <SteamWindow
              key={index}
              className="flex flex-col p-2 gap-1 items-center w-max"
            >
